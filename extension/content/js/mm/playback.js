...
function() {
    ...
    this.resetPlayBack = function() {
        if (($('runningStatus').innerHTML.indexOf('Waiting for tests...') != -1) && ($('playback').src.indexOf("img/playbackstop.png") != -1) && mozmill.ui.playback.running) {
            $('playback').src = 'img/playback.png';
            mozmill.ui.playback.running = false;
    ...
    this.setPlaying = function() {
        ...
        mozmill.ui.playback.running = true;
    ...
    this.sendPlayBack = function(uuid, suiteOnly) {
        ...
        mozmill.ui.domexplorer.domExplorerOff();
        mozmill.ui.assertexplorer.assertExplorerOff();
        ...
        //else play every suite in the IDE
            ...
            var suites = mozmill.remote.$('ideForm').childNodes;
        }
            ...
            if (suites[i].hasChildNodes()) {
                for (var j = 1; j < suites[i].childNodes.length; j++) {
                    ...
                    if (appending == true) {
                        ...
                        if (mozmill.remote.$(suites[i].childNodes[j].id + 'params') != null) {
                            actionObj.method = mozmill.remote.$(suites[i].childNodes[j].id + 'method').value;
                            actionObj.params = eval('(' + mozmill.remote.$(suites[i].childNodes[j].id + 'params').value + ')');

                        }
                        //if its a standard UI element build the params
                        else {
                            var si = mozmill.remote.$(suites[i].childNodes[j].id + 'method').selectedIndex;
                            actionObj.method = mozmill.remote.$(suites[i].childNodes[j].id + 'method')[si].value;

                            var paramsObj = {};
                            paramsObj.uuid = suites[i].childNodes[j].id;

                            if (mozmill.registry.methods[actionObj.method].locator) {
                                var si = mozmill.remote.$(suites[i].childNodes[j].id + 'locatorType').selectedIndex;
                                paramsObj[mozmill.remote.$(suites[i].childNodes[j].id + 'locatorType')[si].value] = mozmill.remote.$(suites[i].childNodes[j].id + 'locator').value;

                            }
                            if (mozmill.registry.methods[actionObj.method].option) {
                                var si = mozmill.remote.$(suites[i].childNodes[j].id + 'optionType').selectedIndex;
                                paramsObj[mozmill.remote.$(suites[i].childNodes[j].id + 'optionType')[si].value] = mozmill.remote.$(suites[i].childNodes[j].id + 'option').value;
                        ...
                        mozmill.remote.$(suites[i].childNodes[j].id).style.background = 'lightyellow';
                    ...
                    if ((mozmill.remote.$('playCascade').checked == false) && (typeof(uuid) != 'undefined')) {
                        appending = false;
        ...
        var respRun = function(str) {
            //setTimeout('mozmill.remote.$(\'playback\').src = \'img/playback.png\'', 3000);
            mozmill.ui.playback.running = true;
        ...
        doCall = function() {
            var z = fleegix.xhr.doPost(respRun, '/mozmill-jsonrpc/', json_string);

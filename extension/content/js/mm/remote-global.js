...
if (!opener) {
    document.location = '/mozmill-serv/start.html';
...
var toggleRec = function() {
    ...
    if ($('record').src.indexOf("img/record.png") != -1) {
        mozmill.ui.domexplorer.domExplorerOff();
        mozmill.ui.assertexplorer.assertExplorerOff();
        mozmill.ui.recorder.recordOn();
        mozmill.window.focus();
...
var togglePlay = function() {
    if ($('playback').src.indexOf("img/playback.png") != -1) {
        mozmill.ui.results.writeStatus("Playing IDE Actions...");
        mozmill.controller.continueLoop();
        mozmill.ui.playback.sendPlayBack();
...
var toggleExplore = function() {
    if ($('explorer').src.indexOf("img/xon.png") != -1) {
        ...
        if (mozmill.ui.recorder.recordState == true) {
            toggleRec();
        }
        $('domExp').style.visibility = 'visible';
        $('domExp').innerHTML = '';
        mozmill.ui.domexplorer.domExplorerOn();
        opener.window.focus();
...
var toggleAExplore = function() {
    if ($('assertx').src.indexOf("img/axon.png") != -1) {
        ...
        $('domExp').style.visibility = 'visible';
        $('domExp').innerHTML = '';
        mozmill.ui.assertexplorer.assertExplorerOn();
        opener.window.focus();

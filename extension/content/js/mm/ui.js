...
function() {
    ...
    this.toggleCollapse = function(id) {
        if (mozmill.remote.$(id).style.height == '18px') {
            mozmill.remote.$(id).style.height = '';

        }
        else {
            mozmill.remote.$(id).style.height = '18px';
        }
    ...
    this.toggleBreak = function() {
        var breakCheckBox = mozmill.remote.$('toggleBreak');
        if (breakCheckBox.checked) {
            mozmill.stopOnFailure = true;

        }
        else {
            mozmill.stopOnFailure = false;
    ...
    this.getContMethodsUI = function() {
        var str = '';
        for (var i in mozmill.controller) {
            if (i.indexOf('_') == -1) {
                str += "," + i;
            }
        }

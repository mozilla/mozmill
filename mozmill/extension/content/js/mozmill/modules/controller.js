...
mozmill.controller = new function () {
  ...
  this._getDocumentStr = function () { return mozmill.testWindowStr + '.document'; }
  this._getWindowStr = function() { return mozmill.testWindowStr; }
  this._getDocument = function () { return mozmill.testWindow.document; }
  this._getWindow = function() { return mozmill.testWindow; }
  ...
  this._lookupDispatch = function (param_object){
    ...
    if(typeof param_object.jsid != "undefined") {
      ...
      var jsid; 
      if ((param_object.jsid.indexOf('mozmill.testWindow') != -1) || (param_object.jsid.indexOf('_w') != -1)){
  ...
  this.handleVariable = function (val){
    ...
    if (matches) {
      for (var i = 0; i < matches.length; i++){
        var m = matches[i];
        if (mozmill.varRegistry.hasKey(m)){
          ret = val.replace(m, mozmill.varRegistry.getByKey(m));
        }
        //if it doesn't exist and contains the string random we create it (username or pass etc)
        else if (m.indexOf('random') != -1){
          ret = val.replace(m, mozmill.varRegistry.addItemCreateValue(m));
        }
      }
    }
    return ret;
...
/************************************
/* User facing mozmill functionality
/************************************/
  ...
  this.continueLoop = function (){
    ...
    cont = function(){
        ...
        try{
          ...
          document.domain = mozmill.docDomain;
        }
...
//      $('loopLink').innerHTML = 'Pause Loop';
  ...
  this.open = function (param_object) {
    ...
    //head to a new page
    try{ mozmill.testWindow.location = param_object.url; }
    catch(err){}
    //Turn off loop until the onload for the iframe restarts it
      done = function(){
        mozmill.controller.waits.forPageLoad();
        return true;
      }
  ...
  this.dragDrop = function (param_object){   
    ...
    var webApp = mozmill.testWindow;
    mozmill.events.triggerMouseEvent(webApp.document.body, 'mousemove', true, mouseDownPos[0], mouseDownPos[1]);
    mozmill.events.triggerMouseEvent(dragged, 'mousedown', true);
    mozmill.events.triggerMouseEvent(webApp.document.body, 'mousemove', true, mouseUpPos[0], mouseUpPos[1]);
    mozmill.events.triggerMouseEvent(dragged, 'mouseup', true);
    mozmill.events.triggerMouseEvent(dragged, 'click', true);
  ...
  this.dragDropXY = function (param_object){
    ...
    var p = param_object;
    var webApp = mozmill.testWindow;
    mozmill.events.triggerMouseEvent(webApp.document.body, 'mousemove', true, p.source[0], p.source[1]);
    mozmill.events.triggerMouseEvent(webApp.document.body, 'mousedown', true);
    mozmill.events.triggerMouseEvent(webApp.document.body, 'mousemove', true, p.destination[0], p.destination[1]);
    mozmill.events.triggerMouseEvent(webApp.document.body, 'mouseup', true);
    mozmill.events.triggerMouseEvent(webApp.document.body, 'click', true);

    return true;
  };
   
  //Functions for interacting with the mozmill variable storage
  ...
  this.storeURL = function(param_object){
    ...
    if (linkNode){
      mozmill.varRegistry.addItem('{$'+param_object.link +'}',linkNode.href);
      return true;
    }
    else{
      return false;
  ...
  this.mousedown = function (param_object){
      ...
      mozmill.events.triggerMouseEvent(mupElement, 'mousedown', true);  
  ...
  this.mouseup = function (param_object){
    ...
    mozmill.events.triggerMouseEvent(mdnElement, 'mouseup', true);
  ...
  this.mouseout = function (param_object){
    var mdnElement = this._lookupDispatch(param_object);
    mozmill.events.triggerMouseEvent(mdnElement, 'mouseout', true);
  ...
  this.reWriteAlert = function(param_object){
    mozmill.rwAlert = true;
    mozmill.testWindow.alert = function(s){
      mozmill.ui.results.writeResult("<br>Alert: <b><font color=\"#fff32c\">" + s + "</font>.</b>");     
    };
      ...
      for (var i=0;i<iframeCount;i++){
          try{
  	        ...
  	        rwaRecurse(iframeArray[i]);
           	...
           	mozmill.ui.results.writeResult('There was a problem rewriting alert on one of your iframes, is it cross domain?' +
  					  'Binding to all others.' + error);     
          }
        }
    }
    rwaRecurse(mozmill.testWindow);
 ...
 /* this.reWriteOpen = function(param_object){
    ...
    mozmill.openWindow = mozmill.testWindow.open;
    
    mozmill.testWindow.open = function(){
      //alert('asda');
      var args = Array.prototype.slice.call(arguments);
      var p = mozmill.openWindow.apply(window, args);
      var date = new Date();
      var wid = date.getTime();
      mozmill.varRegistry.addItem(wid,p);
      return p;
    };
      ...
      for (var i=0;i<iframeCount;i++){
          try{
  	        ...
  	        rwaRecurse(iframeArray[i]);
           	...
           	mozmill.ui.results.writeResult('There was a problem rewriting open on one of your iframes, is it cross domain?' +
  					  'Binding to all others.' + error);     
          }
        }
    }
  ...
  this.setTestWindow = function(param_object){
    var res = eval ('mozmill.testWindow ='+ param_object.path +';');
  ...
  this.findElementBy = function (locatorType, locator, inDocument, inWindow) {
    ...
    if (! locatorFunction) {
      mozmill.ui.results.writeResult("Unrecognised locator type: '" + locatorType + "'");
    }
...
this.findElement = function (locator) {
    ...
    var findElementRecursive = function(w, locatorType, locatorString){
      ...
      element = mozmill.controller.findElementBy(locatorType, locatorString, w.document, w);   
      if (!element){
    ...
    findElementRecursive(mozmill.testWindow, locatorType, locatorString);
    if (e) { return e; }

    // Element was not found by any locator function.
    //mozmill.ui.results.writeResult("Element " + locator + " not found");
};
...
this.locateElementByDomTraversal = function (domTraversal, document, window) {
  ...
  try {
    ...
    mozmill.ui.results.writeResult("dom Traversal, element not found.");
  }
...
this._findElementByTagNameAndText = function (
  ...
  var elements = inDocument.getElementsByTagName(tagName);
  for (var i = 0; i < elements.length; i++) {
    if (mozmill.events.getText((elements[i]) == text)) {
      return elements[i];
    }
  }
  return null;
...
this.locateElementByLinkText = function (linkText, inDocument, inWindow) {
  ...
  for (var i = 0; i < links.length; i++) {
    var element = links[i];
    if (PatternMatcher.matches(linkText, mozmill.events.getText(element))) {
      return element;
    }
  }
  return null;
...
OptionLocatorFactory.prototype.OptionLocatorByLabel = function (label) {
  ...
  this.findOption = function(element) {
    ...
    mozmill.ui.results.writeResult("Option with label '" + this.label + "' not found");
  };
...
OptionLocatorFactory.prototype.OptionLocatorByValue = function (value) {
  ...
  this.findOption = function(element) {
    ...
    mozmill.ui.results.writeResult("Option with value '" + this.value + "' not found");
  };
...
OptionLocatorFactory.prototype.OptionLocatorByIndex = function (index) {
  ...
  if (isNaN(this.index) || this.index < 0) {
    ...
    mozmill.ui.results.writeResult("Illegal Index: " + index);
    
  }
  this.findOption = function (element) {
    if (element.options.length <= this.index) {        
      mozmill.ui.results.writeResult("Index out of range.  Only " + element.options.length + " options available");
    }
...
OptionLocatorFactory.prototype.OptionLocatorById = function (id) {
  ...
  this.findOption = function(element) {
    ...
    //mozmill.mozmill.ui.results.writeResult(.debug("Option with id '" + this.id + "' not found");
